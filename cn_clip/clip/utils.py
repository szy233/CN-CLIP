...
import torch
from torchvision.transforms import Compose, ToTensor, Normalize, Resize, InterpolationMode, RandomHorizontalFlip, \
    ColorJitter, Lambda
...
def add_random_noise(image, noise_level=0.1):
    noisy_image = image + noise_level * torch.randn(image.size())
    return noisy_image
...
def image_transform(image_size=224):
    transform = Compose([
        ...
        # Lambda(lambda x: add_random_noise(x, noise_level=1.0)),
    ])
    return transform
